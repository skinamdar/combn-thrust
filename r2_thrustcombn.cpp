...
void find_comb(int idx, int *x, int m, int n, int*pos, int *out){
    printf("Inside find_comb with index: %d\n",idx);
    ...
    //printf("index %d has n = %d\n", idx, new_n);
     ...
     // need to make sure answer isn't hardcoded 
    
    printf("ANSWER1! %d %d %d with index: %d \n", x[comb[0] + idx], x[comb[1]+idx], x[comb[2]+idx], idx);
    ...
    while(true){
      //  printf(" inside comb is: %d %d %d, i = nothing\n", comb[0], comb[1], comb[2]);
        ...
        printf("ANSWER %d %d %d with index: %d \n", x[comb[0]+idx], x[comb[1]+idx], x[comb[2]+idx],idx);
...
RcppExport SEXP combn(SEXP x_, SEXP m_, SEXP n_, SEXP nCm_, SEXP pos_)
{
    ...
    thrust::device_vector<int> d_r(nCm * m);
